...
> (E18) / 33 pre-registered attempts — 32 equity across 8 strategy families + a
...
> state-conditioned strategies) is complete — but **the program is NOT closed.** M10 found
> the one design that clears PASS-HR *in-sample* (the Nagel Switch), a forward-paper
> hypothesis rather than a refutation of the terminal claim; and one whole mechanism
> family the program has never touched — **algorithmic chart-pattern detection** — remains
> unstarted (§8, PRD M11). This reads as a progress report, not an obituary.
...
## 3. Results ledger — 8 equity families + a synthesis arc + 1 crypto pilot, 33 attempts, 0 clean high-return passes
...
forward-paper-only), 1 weak PASS-RA (E18 VIX-TS), 33 pre-registered attempts (32 equity
across 8 families + a state-conditioned synthesis arc + 1 crypto pilot).** Full
...
corrects — its own errors. **Thirty-three pre-registered attempts across eight equity
...
**(a) Algorithmic chart-pattern detection — the one untested mechanism family (PRD M11).**
...
fixed-single-strategy space (E/C/X families) is exhausted and M10 is complete: M10-1
(Nagel Switch) is the program's first PASS-HR but IN-SAMPLE-COMPOSED / forward-paper-only,
and M10-2 closed the E2 "c2c mirage" (FAIL). **What remains open:** the algorithmic
chart-pattern family (PRD M11 — unstarted, the next honest experiment, data in hand); and
the Evan-gated levers — M3 Alpaca paper deploy of the three forward-paper candidates
(E6-1× / E18-VIX-TS / M10-1), the one source of uncontaminated evidence; X5
analyst-revision drift (**BLOCKED — needs the $22 FMP feed; the free alternative is
look-ahead-contaminated**); live-money crypto (custody gate). Standing at 33 attempts /
0 clean PASS-HR / 1 in-sample PASS-HR / 1 weak PASS-RA. **The research phase is NOT
declared done.***
